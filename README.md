...
- `OPENAI_CANDIDATE_LIMIT=30` (supported range: 5–60)
...
The OpenAI advisor receives a roster-aware, position-diverse set of up to 30 valid quantitative candidates by default, configurable with `OPENAI_CANDIDATE_LIMIT`. Deterministic evaluation scores the complete available pool first. Candidate selection then removes ambiguous duplicate identities, guarantees up to three representatives for every unfilled starter position, and fills the packet by quantitative score under dynamic position caps. Positions below starter capacity and unfilled starters receive the most depth; filled or surplus positions receive less; early K/DEF targets retain one emergency candidate; positions already at a configured maximum receive none. This prevents one high-volume position from flooding the prompt while allowing the configured packet to reach its useful budget.

Every request also includes the draft's complete snapshotted scoring rules and roster rules, current pick and next-pick geometry, every league roster and its needs, next-turn opponents, data-snapshot references, and configured strategy preferences. Each candidate includes FantasyPros ECR/ADP/provider tier when available plus our league-adjusted projected points, VOR, calculated tier and cliff, scarcity, survival probability, cost of waiting, roster fit, upside/risk, and deterministic score. It uses the Responses API Structured Outputs schema, constrains player IDs to that request's candidate allowlist, defaults to `gpt-5.6-terra` with `low` reasoning effort, and makes no draft mutations. The automatic request happens only once the preceding pick has been saved and our team is on the clock. Successful results and credential-safe failed-attempt markers are cached for that exact draft state, so refreshes do not call the model again. Missing key, timeout, network failure, rate limit, invalid schema, or invented/drafted ID all result in the offline quantitative recommendations with a prominent `AI FALLBACK ACTIVE` warning.
